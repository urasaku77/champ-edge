...
- 対戦履歴・対戦分析画面の「AIフィードバック」ボタンで利用する Anthropic API のキーを設定する
...
- APIキーは [Anthropic Console](https://console.anthropic.com) で発行可能
...
![AIフィードバック画面](/image/readme/ai_feedback.png)
...
- APIキーはアプリ設定または環境変数で事前設定が必要（[Anthropic Console](https://console.anthropic.com) で発行）
