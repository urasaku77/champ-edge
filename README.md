...
画面上部に複数のラジオ/チェックを横並びに配置。表示・集計・絞り込みの切替を行う。

- **表示モード**（ラジオ3択）
  - 「ＫＰと勝率」: 相手KPと勝率
  - 「選出と勝率」: 相手の選出率と選出時勝率
  - 「初手と勝率」: 相手の初手選出率と初手選出時勝率
- **ソート条件**（ラジオ3択）
  - 「KP」: 表示モードの上段の値で並べる
  - 「勝率」: 表示モードの下段の値で並べる
  - 「使用率順」: `stats/ranking.txt` 上位 N 体に表示対象を差し替えて、各ポケモンの値を再計算
- **降順 / 昇順**（ラジオ2択）— 並び方向を切り替え（使用率順では並びは ranking 順に固定）
- **値の表示形式**（ラジオ3択）
  - 「％表示」: `50.0%` のような率表示
  - 「分数表示」: `5/10` のような分子分母表示
  - 「両方表示」: `50.0% (5/10)` のような両方併記。各セル幅を自動で広げる
- **エリア表示**（ラジオ2択）
  - 「スクロール表示」: ウィンドウサイズを 1280x720 に固定し、はみ出した分はスクロール
  - 「全面表示」（デフォルト）: ウィンドウサイズを内容に合わせて拡大
- **表示件数**（ラジオ2択）
  - 「50位まで」（デフォルト） / 「100位まで」
- **メガ統合**チェック（デフォルト ON）
  - ON: メガ進化前後を同一ポケモンとして集計
  - OFF: メガ前後を別ポケモンとして扱う。使用率順 ON 時はランキング各ポケモンの直下にそのメガフォームを挿入表示
- **トップに絞る**チェック + ラジオ（10 / 30 / 50）
  - ON 時、現在のソート結果から `stats/ranking.txt` 上位 N 位に該当しないポケモンを除外（並び順は維持）
  - 「使用率順」と排他関係。一方を有効にすると他方は無効化される
  - ON 中は「100位まで」も使えないため自動で「50位まで」に固定される
...
![アプリ設定](/image/readme/setting_mode.png)
...
- **相手TN OCR自動読み込み（非推奨）**
  - manga-ocr ではなく Tesseract で相手TNを認識する（精度が低い）
- **選出ポケモン OCR自動読込み（要バトルネーム非表示設定）**
  - 選出画面のポケモン名を Tesseract で認識する
...
メニュー「Tesseractセットアップ」から開く。OCR機能（相手TN・パーティ画像読み取り）で利用する Tesseract OCR の設定・インストールを行う。
